---
layout:     post
title:      "IDEA系列（三十七） 远程调试"
subtitle:   ""
date:       2017-04-29 12:00:37
author:     "Crimps"
...
- 添加 Remote Server，如下图
	- ![本地 Tomcat 配置](/img/in-post/remote-debugging-1.jpg)
...
- 如下图，在 Host 添加服务器的 IP 地址：192.168.92.128。
	- ![本地 Tomcat 配置](/img/in-post/remote-debugging-2.jpg)
- 把刚刚复制参数加个前缀，变成：
...
- 编辑 Tomcat 执行程序：`vim /usr/program/tomcat7/bin/catalina.sh`（Windows 是编辑：catalina.bat）
	...
	- 如下图所示：
	- ![远程 Tomcat 配置](/img/in-post/remote-debugging-3.jpg)
...
- 如果可以看到如下图效果，表示已经连接成功了，接下里就是跟往常一样，在本地代码上设置断点，然后你访问远程的地址，触发到该代码自动就会在本地停住。
	- ![开始调试](/img/in-post/remote-debugging-4.jpg)
- 如下图，可以看到调试效果
	- ![开始调试](/img/in-post/remote-debugging-5.jpg)
